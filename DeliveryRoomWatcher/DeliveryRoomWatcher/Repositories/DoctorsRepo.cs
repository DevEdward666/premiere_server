...
namespace DeliveryRoomWatcher.Repositories
{
    public class DoctorsRepo
    {
        ...
        public ResponseModel getdoctorsbyspecialtyandname(PDoctors.PGetDoctorsDetails offset)
        {
            using (var con = new MySqlConnection(DatabaseConfig.GetConnection()))
            {
                con.Open();
                using (var tran = con.BeginTransaction())
                {
                    try
                    {
                        var data = con.Query($@"SELECT doc.* FROM docmaster doc JOIN docspecialty docs ON doc.`specialty`=docs.`spclcode` where doc.specialty=@specialty  AND CONCAT(lastname,' ',firstname,',',middlename) LIKE concat('%',@name,'%') LIMIT @offset",
                          offset, transaction: tran
                            );
